...
export default function CuttingInstructions({ 
  ...
  if (layoutId === 'empty' || pieces.length === 0) {
    return (
      <div className="bg-amber-900/30 border border-amber-205/60 rounded-2xl p-5 text-center">
        <AlertTriangle className="h-8 w-8 text-amber-600 mx-auto mb-2" />
        <h4 className="text-sm font-bold text-amber-200 uppercase tracking-wide">Sin Plan de Guillotina</h4>
  ...
  return (
    <div id="instructions-container" className="space-y-5 text-slate-100">
      {/* Intro Info Banner */}
      <div className="flex items-start gap-3 bg-indigo-900/30/60 border border-indigo-100 rounded-2xl p-4">
      ...
      {/* Timeline steps */}
      <div className="relative border-l border-slate-700 pl-6 ml-3.5 space-y-6">
        
        {/* Step 1: Material Verification */}
        <div className="relative">
          <span className="absolute -left-[35px] top-0.5 flex items-center justify-center h-6 w-6 rounded-full bg-indigo-900/300 text-[10px] font-bold text-white shadow-sm shadow-indigo-100 border border-white">
            1
          </span>
          <div className="space-y-1">
            <h5 className="text-xs font-bold text-slate-100 flex items-center gap-1.5 uppercase tracking-wide">
              <Compass className="h-3.5 w-3.5 text-slate-500" />
              Verificación de Fibra y Alimentación
            </h5>
            <p className="text-xs text-slate-300 leading-relaxed font-semibold">
              Coloque la pila de <span className="font-bold text-slate-50 uppercase">{layout.totalSheetsWithWaste} pliegos</span> en la guillotina. 
              {fiberDirection === 'any' && (
                <span> Dirección de fibra seleccionada: <strong className="bg-slate-800 px-1.5 py-0.5 text-slate-300 font-bold font-mono text-[9.5px] rounded-md border border-slate-205">CUALQUIERA / INDIFERENTE</strong>.</span>
              )}
              {fiberDirection === 'with-width' && (
                <span> Asegúrese de colocar los pliegos de modo que la fibra del papel corra paralela al lado ancho de <strong className="font-mono font-bold text-slate-50">{sheetWidth} cm</strong>.</span>
              )}
              {fiberDirection === 'with-length' && (
                <span> Asegúrese de colocar los pliegos de modo que la fibra corra paralela al largo de <strong className="font-mono font-bold text-slate-50">{sheetLength} cm</strong>.</span>
              )}
            </p>
          </div>
        </div>
          ...
          <div className="space-y-1">
            ...
            {totalDirect > 0 && totalRotated === 0 && (
              <div className="text-xs text-slate-300 space-y-1 leading-relaxed">
                ...
                <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-4 space-y-2 mt-2">
                  <div className="flex items-start gap-2 text-slate-100">
                    <ArrowRight className="h-4 w-4 mt-0.5 text-indigo-400 shrink-0" />
                    <div className="font-semibold">
                      <strong>Corte Horizontal:</strong> Divida el pliego en tiras horizontales de{' '}
                      <span className="bg-indigo-900/30 text-indigo-700 border border-indigo-100 font-mono font-bold px-1.5 py-0.5 rounded">
                        {directPieces[0].height} cm
                      </span>{' '}
                      de alto.
                    </div>
                  </div>
                  <div className="flex items-start gap-2 text-slate-100 pt-2 border-t border-dashed border-slate-700">
            ...
            {totalDirect > 0 && totalRotated > 0 && (
              <div className="text-xs text-slate-300 space-y-1 leading-relaxed">
                <p className="font-semibold text-slate-605">
                  Dado que está utilizando un <strong className="font-bold text-slate-100">Corte Mixto Avanzado</strong> para aprovechar el excedente de mermas, divida el pliego en dos bloques de guillotina principales:
                </p>
                
                <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-4 space-y-3 mt-2">
                  <div>
                    <h6 className="font-bold text-slate-100 uppercase tracking-wide flex items-center gap-1.5">
                      <span className="h-2 w-2 rounded-full bg-indigo-900/300" />
        ...
        <div className="relative font-sans">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-300 leading-relaxed font-semibold">
              Cada pliego rinde exactamente <span className="font-bold text-slate-100">{layout.piecesPerSheet} piezas</span> terminadas.
              Al procesar los <span className="font-bold text-slate-100">{layout.totalSheetsWithWaste} pliegos</span> cargados en la orden obtendrá un estimado final de <span className="font-bold text-slate-50">{layout.piecesPerSheet * layout.totalSheetsWithWaste} volantes/piezas</span> (cubriendo el objetivo de {targetQuantity} unidades con {wastePercentage}% de demasía técnica por mermas o montaje).
            </p>
          </div>
        </div>

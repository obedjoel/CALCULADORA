...
export default function PresetBar({
  ...
  return (
    <div id="presets-panel" className="bg-slate-900 border-4 border-black rounded-none p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] space-y-5">
      <div className="flex items-center gap-2 pb-3 border-b-4 border-black">
        <Zap className="h-5 w-5 text-white" fill="#EFFF00" />
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-none text-xs text-left cursor-pointer border-2 border-black font-bold transition-all ${
                    isActive
                      ? 'bg-[#EFFF00] text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
                      : 'bg-slate-900 hover:bg-neutral-100 text-white shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]'
                  }`}
                  title={p.description}
                >
                  <div className="font-extrabold block uppercase tracking-tight">{p.name}</div>
                  <div className={`text-[10px] font-mono ${isActive ? 'text-white/80' : 'text-neutral-500'}`}>
